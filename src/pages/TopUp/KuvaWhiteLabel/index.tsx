...
const KWL = () => {
  ...
  const { list: { items, count }, listFetchFinished } = useAppSelector(state => state.topUp.kwl)

  useEffect(() => {
    (async () => {
      await dispatch(fetchTopUpKWLList(getQueryString(pagination)))
    })()
  }, [])
  ...
  const fetchData = async (params: { [key: string]: string | number } = pagination, isApply: boolean = false) => {
    ...
    await dispatch(fetchTopUpKWLList(queryString))
  ...
  const headData = ['Created', 'Reference', 'Type', 'Status', 'From', 'To', 'Details']
  const dataKeys = ['createdDate', 'reference', 'requestType', 'status', 'from', 'to', 'detailsBtn']
